<?php
    ...
    if($pass && $mail && $num) {
        ...
        $sqlSequence = "CREATE SEQUENCE idMaitre START 1;";
        $querySequence = pg_query($conn, $sqlSequence);

        $sqlVal = "SELECT nextval('idMaitre');
                    SELECT currval('idMaitre');";
        $queryVal = pg_query($conn, $sqlVal);
        $affiche = pg_fetch_array($queryVal);
        $value = $affiche[0];

        $sqlInsertion = "insert into maitre (id, password, mail, num) values ('".$value."', '".$_POST['pwd']."', '".$_POST['mail']."', '".$_POST['num']."')";
